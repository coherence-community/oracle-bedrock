...
import com.oracle.tools.runtime.docker.commands.Run;
import com.oracle.tools.runtime.docker.machine.DockerMachine;
import com.oracle.tools.runtime.docker.machine.DockerMachinePlatform;
...
import static com.oracle.tools.runtime.docker.machine.DockerMachineTest.hasDockerMachine;
import static com.oracle.tools.runtime.docker.machine.DockerMachineTest.killMachine;
import static com.oracle.tools.runtime.docker.machine.DockerMachineTest.removeMachine;
 ...
 * This requires the <code>oracle.tools.container.tests.jre</code> System property
...
public class DockerFunctionalTest extends AbstractFunctionalTest
{
    ...
    public static void createPlatform()
    {
        ...
        Docker docker = Docker.auto()
                              .withBaseImage(JavaApplication.class, javaImageName);

        platformName = "oracle-tools-" + System.currentTimeMillis();
    ...
    public void shouldCreateImageAndRunContainer() throws Exception
    {
        ...
        try (Application imageApp = platform.launch(Build.fromDockerFile()
                                                         .withTags(tag),
                                                    WorkingDirectory.at(workingDirectory)))
        {
            ...
            try (Application containerApp = platform.launch(Run.image(image)
                                                               .interactive(),
                                                            Argument.of("cat"),
                                                            ...
                                                            Console.of(console)))
            {
                ...
                boolean hasMessage = console.getCapturedOutputLines().stream()
                                            .filter((line) -> line.equals(message))
                                            .findFirst()
                                            .isPresent();
    ...
    public void shouldRunJavaApplicationThatListensOnSocketAndConnectToNATedPort() throws Exception
    {
        ...
        try (JavaApplication server = platform.launch(JavaApplication.class,
                                                      ClassName.of(SocketEchoServer.class),
                                                      // --- we use Ports.capture() to capture the Docker port NAT'ing
                                                      SystemProperty.of(SocketEchoServer.PROPERTY_SERVER_PORT,
                                                                        availablePorts, Ports.capture())))
        {
